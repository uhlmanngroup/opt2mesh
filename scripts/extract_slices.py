...
def parse_args():
    ...
    parser.add_argument(
        "--first", default=20, type=int, help="The first slice to extract"
    )
    parser.add_argument(
        "--last", default=500, type=int, help="The last slice to extract"
    )
    parser.add_argument(
        "--n_slices", default=5, type=int, help="The last slice to extract"
    )
...
if __name__ == "__main__":
    ...
    for x in np.linspace(
        start=args.first, stop=min(args.last, h), num=args.n_slices, dtype=int
    ):
    ...
    for y in np.linspace(
        start=args.first, stop=min(args.last, w), num=args.n_slices, dtype=int
    ):
    ...
    for z in np.linspace(
        start=args.first, stop=min(args.last, d), num=args.n_slices, dtype=int
    ):

...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __getitem__(self, idx):
        ...
        raw_patch_transformed = self._transform_patches(
            self.raws, raw_idx, self.raw_transform
        )
        ...
        if self.phase == "test":
            ...
            label_patch_transformed = self._transform_patches(
                self.labels, label_idx, self.label_transform
            )
            if self.weight_maps is not None:
                ...
                weight_patch_transformed = self._transform_patches(
                    self.weight_maps, weight_idx, self.weight_transform
                )
                return (
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._raw_slices = self._build_slices(
            raw_datasets[0], patch_shape, stride_shape
        )
        if label_datasets is None:
            ...
            self._label_slices = self._build_slices(
                label_datasets[0], patch_shape, stride_shape
            )
        ...
        if weight_dataset is None:
            ...
            self._weight_slices = self._build_slices(
                weight_dataset[0], patch_shape, stride_shape
            )

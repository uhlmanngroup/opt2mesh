    ...
    def __init__(
        ...
        **kwargs,
        ...
        def ignore_predicate(raw_label_idx):
            ...
            non_ignore_counts = np.array(
                [np.count_nonzero(patch != ii) for ii in ignore_index]
            )
            non_ignore_counts = non_ignore_counts / patch.size
            return (
                np.any(non_ignore_counts > threshold)
...
class EmbeddingsSliceBuilder(FilterSliceBuilder):
    ...
    def __init__(
        ...
        threshold=0.8,
        slack_acceptance=0.01,
        patch_max_instances=48,
        patch_min_instances=5,
        **kwargs,
    ):
        super().__init__(
            raw_datasets,
        ...
        def ignore_predicate(raw_label_idx):
            ...
            if num_instances <= patch_max_instances:
                ...
                return (
                    num_instances >= patch_min_instances
                    or rand_state.rand() < slack_acceptance
                )
